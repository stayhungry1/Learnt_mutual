...
class GeneralizedRCNN(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        compressaiargs_lambda = 0.05 #指令里
        ...
        self.i_step_count = 0
        compressai_logdir = '/media/data/liutie/VCM/rcnn/VCMbelle_0622/VCM/tensorboard_belle/EXP_4chinput_P2MSE_nobpploss_08021550/'
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        for channel_idx in range(cai_input_tensor_new.size()[1] // 4):
            # d = cai_input_tensor_new[:, channel_idx: (channel_idx + 1), :, :]
            d = cai_input_tensor_new[:, channel_idx * 4: (channel_idx * 4 + 4), :, :]
            # d = d.repeat(1, 3, 1, 1)  # channel x 3 [1, 3, h, w]
            d = d.to(device)
            # d = (d + 12.5) / 25.0  # 2张图是-12.3-12.3
            d = (d - self.guiyihua_min) / self.guiyihua_scale
            print(d.size(), '-------------------belle input (P2) size')
            # d_p4 = cai_input_tensor_p4_new[:, channel_idx: (channel_idx + 1), :, :]
            d_p4 = cai_input_tensor_p4_new[:, channel_idx * 4: (channel_idx * 4 + 4), :, :]
            ...
            print("i_step: %d, channelidx: %d/%d, max/min_P2(belle input): %8.4f/%8.4f, max/min_P2(belle output): %8.4f/%8.4f"
                  % (self.i_step_count, channel_idx, cai_input_tensor_new.size()[1] // 4, torch.max(d), torch.min(d), torch.max(net_belle_output["x_hat"]), torch.min(net_belle_output["x_hat"])))

            #net_belle_output["x_hat"]小于0置为0，大于1置为1

            if (self.i_step_count % self.belle_savetensorboardfreq == 0) and (channel_idx == 0):
                i_select_channel = random.randint(0, 3)
