...
class GeneralizedRCNN(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        compressaiargs_lambda = 0.001 #指令里
        ...
        self.i_step_count = 0
        compressai_logdir = '/media/data/liutie/VCM/rcnn/VCMbelle_0622/VCM/tensorboard_belle/EXP07302140/'
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        # for channel_idx in range(1):  # 0-256通道
            ...
            d = d.repeat(1, 3, 1, 1)  # channel x 3 [1, 3, h, w]
            d = d.to(device)
            d = (d + 12.5) / 25.0  # 2张图是-12.3-12.3
            d_p4 = cai_input_tensor_p4_new[:, channel_idx: (channel_idx + 1), :, :]
            d_p4 = d_p4.repeat(1, 3, 1, 1)  # channel x 3 [1, 3, h, w]
            d_p4 = d_p4.to(device)
            d_p4 = (d_p4 + 12.5) / 25.0  # 2张图是-12.3-12.3
            net_belle_output = self.net_belle(d)
            # x_enh = net_belle_output["x_hat"] #不走质量增强
            out_criterion = self.belle_criterion(net_belle_output, d_p4)
            # out_criterion = self.CAI["criterion"](out_net, cai_input_tensor, x_hat, x_l, x_enh)
            out_criterion["loss"].backward()
            # if self.CAI["clip_max_norm"] > 0:
            if self.belle_clip_max_norm > 0:
                # torch.nn.utils.clip_grad_norm_(self.CAI["model"].parameters(), self.CAI["clip_max_norm"])
                torch.nn.utils.clip_grad_norm_(self.net_belle.parameters(), self.belle_clip_max_norm)
            # self.CAI["optimizer"].step()
            self.optimizer_belle.step()

            # aux_loss = self.CAI["model"].aux_loss()
            aux_loss = self.net_belle.aux_loss()
            aux_loss.backward()
            # self.CAI["aux_optimizer"].step()
            ...
            # print("i_step: %d, channelidx: %d/%d, max/min_P2(belle input): %8.4f/%8.4f, max/min_P2(belle output): %8.4f/%8.4f"
            #       % (self.i_step_count, channel_idx, cai_input_tensor_new.size()[1], torch.max(d), torch.min(d), torch.max(net_belle_output["x_hat"]), torch.min(net_belle_output["x_hat"])))
            print("i_step: %d, channelidx: %d/%d, max/min_P4(belle input): %8.4f/%8.4f, max/min_P4(belle output): %8.4f/%8.4f"
                  % (self.i_step_count, channel_idx, cai_input_tensor_new.size()[1], torch.max(d_p4), torch.min(d_p4), torch.max(net_belle_output["x_hat"]), torch.min(net_belle_output["x_hat"])))
            if (self.i_step_count % self.belle_savetensorboardfreq == 0) and (channel_idx == 0):
                ...
                self.belle_writer.add_image('P2_GT', d[0, 0:1, :, :], global_step=self.i_step_count, dataformats='CHW')  # dataformats='HWC')
                self.belle_writer.add_image('P4_GT', d_p4[0, 0:1, :, :], global_step=self.i_step_count, dataformats='CHW')  # dataformats='HWC')
