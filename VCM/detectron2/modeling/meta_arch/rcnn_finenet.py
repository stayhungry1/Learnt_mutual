...
class GeneralizedRCNN(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.i_step_count = 0
        compressai_logdir = '/media/data/liutie/VCM/rcnn/VCMbelle_0622/VCM/tensorboard_belle/EXP_cheng2020anchor_256chinput_P4MSE_ft5Wimgtrain5999_lambda1_N192_eachdnorm_fineoutP2MSE_08112130/'
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        print("train_loss:%8.4f, PSNR: %8.4f, max/min_P2(GT): %8.4f/%8.4f max/min_P2(Finenet input): %8.4f/%8.4f, max/min_P2(FineNet output): %8.4f/%8.4f" % (loss_l2, psnr_temp1, torch.max(fake_image_f_GT), torch.min(fake_image_f_GT),torch.max(up_image), torch.min(up_image), torch.max(fake_image_f), torch.min(fake_image_f)))
