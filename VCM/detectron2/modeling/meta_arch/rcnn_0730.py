...
class GeneralizedRCNN(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        #self.optimizer, aux_optimizer = configure_optimizers(net, args)
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        # print(features["p2"].shape) # [8， 256， 160， 288]
        # cai_input_tensor = torch.zeros([1, 256, 128, 128]).to(device)
        cai_input_tensor = features["p2"] #float32
        print(cai_input_tensor.size(), '-------------------feature["p2"] size') #[b, 256, 240, 160]
        # cai_input_tensor = torch.tensor(cai_input_tensor, dtype=torch.double)
        # print(cai_input_tensor.dtype)
        ...
        print(cai_input_tensor.size(), '-------------------CAI input size')
        # raise ValueError("stop")
        # train CAI framework
        self.CAI["model"].train()
        self.CAI["IRN_net"].train()
        # En_net.train()
        # for one dataloader
        self.CAI["optimizer"].zero_grad()  # optimizer.zero_grad()
        self.CAI["aux_optimizer"].zero_grad()
        self.CAI["optimizer_IRN"].zero_grad()
        # optimizer_En.zero_grad()

        x_l = IRN_inference(self.CAI["IRN_net"], cai_input_tensor, device)
        out_net = self.CAI["model"](x_l)
        # x_enh = En_net(out_net["x_hat"])
        x_enh = out_net["x_hat"] #不走质量增强
        x_hat = IRN_inference(self.CAI["IRN_net"], x_enh, device, rev=True)
        ...
        if self.CAI["clip_max_norm"] > 0:
            ...
            torch.nn.utils.clip_grad_norm_(self.CAI["IRN_net"].parameters(), self.CAI["clip_max_norm"])
            # torch.nn.utils.clip_grad_norm_(En_net.parameters(), clip_max_norm)
        self.CAI["optimizer"].step()
        self.CAI["optimizer_IRN"].step()
        # optimizer_En.step()
        ...
        ## ywz add done
        # raise ValueError("stop forward CAI")
        # --------------------------------------------------
