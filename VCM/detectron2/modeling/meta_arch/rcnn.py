...
class GeneralizedRCNN(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.belle_criterion_p2 = RateDistortionLoss_p2(compressaiargs_lambda)
        self.i_step_count = 0
        # compressai_logdir = '../../liutie_save/tensorboard_belle/EXP_cheng2020anchor_256chinput_P2inP3outMSE_P2zeroyouxiajiao256_lambda1_N192_7imgtrain_eachdnorm_09051830/'
        # compressai_logdir = '../../liutie_save/tensorboard_belle/EXP_cheng2020anchor_256chinput_P2inP3outMSE_P2zeroyouxiajiao256_lambda1_N192_7imgtrainft9999_small5Wtrain_eachdnorm_09052300/'
        ...
        # compressai_logdir = '../../liutie_save/tensorboard_belle/EXP_cheng2020anchor_256chinput_P2inP2P3outMSE_P2zeroyouxiajiao256_lambda1_N192_7imgtrain_eachdnorm_09141000/'
        compressai_logdir = '../../liutie_save/tensorboard_belle/EXP_cheng2020anchor_256chinput_P2inP2P3outMSE_P2zeroyouxiajiao256_lambda1_N192_7imgtrainft14999_small5Wtrain_eachdnorm_09141500/'
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        d_originalsize_p3 = cai_input_tensor_p3
        #normlize p2 and p3
        ...
        d = d.to(device)
        d_p3 = d_p3.to(device)
        d_originalsize_p2 = d_originalsize_p2.to(device)
        d_originalsize_p3 = d_originalsize_p3.to(device)
        ...
        out_criterion_p2 = self.belle_criterion_p2(net_belle_output, d)
        out_criterion_p3 = self.belle_criterion(net_belle_output, d_p3)
        out_criterion_p2p3 = out_criterion_p2["loss"] + out_criterion_p3["loss"]
        # out_criterion["loss"].backward()
        out_criterion_p2p3.backward()
        ...
        psnr_temp_p2 = 10 * math.log10(1 / out_criterion_p2["mse_loss"].item())
        psnr_temp_p3 = 10 * math.log10(1 / out_criterion_p3["mse_loss"].item())
        ...
        mse_temp = define_mse(d_output_p3, d_originalsize_p3)
        psnr_temp_originalsize_p3 = 10 * math.log10(1 / mse_temp)
        ...
        define_mse = nn.MSELoss()
        mse_temp = define_mse(d_output_p2, d_originalsize_p2)
        psnr_temp_originalsize_p2 = 10 * math.log10(1 / mse_temp)

        print(
            f'[P2] Loss: {out_criterion_p2["loss"].item():.3f} |'
            f'\tMSE loss: {out_criterion_p2["mse_loss"].item():.3f} |'
            f'\tBpp loss: {out_criterion_p2["bpp_loss"].item():.2f} |'
            f"\tAux loss: {aux_loss.item():.2f}"
            f'\tPSNR: {psnr_temp_p2:.3f} |'
            f'\tPSNR_orisize: {psnr_temp_originalsize_p2:.3f} |'
        )
        print(
            f'[P3] Loss: {out_criterion_p3["loss"].item():.3f} |'
            f'\tMSE loss: {out_criterion_p3["mse_loss"].item():.3f} |'
            f'\tBpp loss: {out_criterion_p3["bpp_loss"].item():.2f} |'
            f"\tAux loss: {aux_loss.item():.2f}"
            f'\tPSNR: {psnr_temp_p3:.3f} |'
            f'\tPSNR_orisize: {psnr_temp_originalsize_p3:.3f} |'
        )
        ...
        print("i_step: %d, max/min_P2(cheng input): %8.4f/%8.4f, max/min_P3(cheng output): %8.4f/%8.4f, max/min_P3(cheng output): %8.4f/%8.4f"
              % (self.i_step_count, torch.max(d), torch.min(d), torch.max(net_belle_output["x_hat"]), torch.min(net_belle_output["x_hat"]), torch.max(net_belle_output["x_hat_p2"]), torch.min(net_belle_output["x_hat_p2"])))

        if (self.i_step_count % self.belle_savetensorboardfreq == 0): # and (channel_idx == 0):
            i_select_channel = random.randint(0, 255)
            self.belle_writer.add_scalar("training/Loss", out_criterion_p2p3, global_step=self.i_step_count)
            self.belle_writer.add_scalar("training/Loss_P2", out_criterion_p2["loss"], global_step=self.i_step_count)
            self.belle_writer.add_scalar("training/MSE loss_P2", out_criterion_p2["mse_loss"], global_step=self.i_step_count)
            self.belle_writer.add_scalar("training/Bpp loss_P2", out_criterion_p2["bpp_loss"], global_step=self.i_step_count)
            self.belle_writer.add_scalar("training/Loss_P3", out_criterion_p3["loss"], global_step=self.i_step_count)
            self.belle_writer.add_scalar("training/MSE loss_P3", out_criterion_p3["mse_loss"], global_step=self.i_step_count)
            self.belle_writer.add_scalar("training/Bpp loss_P3", out_criterion_p3["bpp_loss"], global_step=self.i_step_count)
            self.belle_writer.add_scalar("training/Aux loss", aux_loss.item(), global_step=self.i_step_count)
            self.belle_writer.add_scalar("training/PSNR_P2", psnr_temp_p2, global_step=self.i_step_count)
            self.belle_writer.add_scalar("training/PSNR_orisize_P2", psnr_temp_originalsize_p2, global_step=self.i_step_count)
            self.belle_writer.add_scalar("training/PSNR_P3", psnr_temp_p3, global_step=self.i_step_count)
            self.belle_writer.add_scalar("training/PSNR_orisize_P3", psnr_temp_originalsize_p3, global_step=self.i_step_count)
            ...
            self.belle_writer.add_image('netcheng_outputP2', net_belle_output["x_hat_p2"][0, i_select_channel:(i_select_channel+1), :, :], global_step=self.i_step_count, dataformats='CHW')  # dataformats='HWC')
            self.belle_writer.add_image('netcheng_outputP3', net_belle_output["x_hat"][0, i_select_channel:(i_select_channel+1), :, :], global_step=self.i_step_count, dataformats='CHW')  # dataformats='HWC')
