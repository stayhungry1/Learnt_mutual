...
class Eval:
    def __init__(self, settings, index) -> None:
        ...
        self.path_bppsave = '../../liutie_save/output/cheng_P2inP3outzeropad128_bpp_lambda1chu16_finenet_iter37999.json'
        # self.path_bppsave = '../../liutie_save/output/cheng_P2inP3outzeropad128_bpp_lambda4_finenet_iter37999.json'
        # self.path_bppsave = '../../liutie_save/output/cheng_P2inP3outzeropad128_bpp_lambda8_finenet_iter37999.json'
        self.bpp_test5000 = {}
    ...
    def _feature_coding(self, inputs, fname_temp):
        ...
        ###pad
        d_p4, h_new_p4_left, h_new_p4_right, w_new_p4_left, w_new_p4_right = Pfeature_zeropad_youxiajiao(d_p4, 32) #Pfeature_zeropad_youxiajiao256(d_p4, 32)
        d_p5, h_new_p5_left, h_new_p5_right, w_new_p5_left, w_new_p5_right = Pfeature_zeropad_youxiajiao(d_p5, 16) #Pfeature_zeropad_youxiajiao128(d_p5, 16)
        ...
        net_belle_output = self.model.net_belle(d_p4)
        print(net_belle_output["x_hat"].size(), '-------------------Cheng output (P5) size')
        d_output = Pfeature_zeropad_youxiajiao128_reverse(net_belle_output["x_hat"], h_new_p5_left, h_new_p5_right, w_new_p5_left, w_new_p5_right)
        print('max/min_P4(GT)(Cheng input): %8.4f/%8.4f, max/min_P5(Cheng output): %8.4f/%8.4f'
              % (torch.max(d_p4), torch.min(d_p4), torch.max(d_output), torch.min(d_output)))
        ...
        loss_l2 = l_l2(d_output_p4, d_originalsize_p4)
        psnr_temp1 = 10 * math.log10(1 / loss_l2)

        up_image = Pfeature_zeropad_youxiajiao128_reverse(up_image, h_new_p4_left, h_new_p4_right, w_new_p4_left, w_new_p4_right)
        loss_l2_0 = l_l2(up_image, d_originalsize_p4)
        psnr_temp1_0 = 10 * math.log10(1 / loss_l2_0)
        dpsnr_temp = psnr_temp1 - psnr_temp1_0

        up_image_P5GT = upsample(d_p5)
        up_image_P5GT = Pfeature_zeropad_youxiajiao128_reverse(up_image_P5GT, h_new_p4_left, h_new_p4_right, w_new_p4_left, w_new_p4_right)
        loss_l2_P5GT = l_l2(up_image_P5GT, d_originalsize_p4)
        psnr_temp1_P5GT = 10 * math.log10(1 / loss_l2_P5GT)

        heigh_temp = self.height_temp
        width_temp = self.width_temp
        numpixel_temp = self.numpixel_temp
        out_criterion = self.criterion(net_belle_output, d_p5, heigh_temp, width_temp) #net_belle_output和d为pad后的
        ...
        define_mse = nn.MSELoss()
        out_criterion["mse_loss"] = define_mse(d_output, d_originalsize_p5)
        psnr_temp = mse2psnr(out_criterion["mse_loss"])
        print('bpp: %8.4f, MSE: %8.4f, PSNR: %8.4f' % (out_criterion["bpp_loss"].item(), out_criterion["mse_loss"].item(), psnr_temp))
        ...
        print("FINENET MSE:%8.4f, (ori)dpsnr/psnr/psnr0: %8.4f/%8.4f/%8.4f, psnr_useP3GT: %8.4f, max/min_P2(GT): %8.4f/%8.4f, max/min_P3up(Finenet input): %8.4f/%8.4f, max/min_P2(FineNet output): %8.4f/%8.4f"
            % (loss_l2, dpsnr_temp, psnr_temp1, psnr_temp1_0, psnr_temp1_P5GT, torch.max(fake_image_f_GT), torch.min(fake_image_f_GT), torch.max(up_image), torch.min(up_image), torch.max(fake_image_f), torch.min(fake_image_f)))
