...
class Eval:
    ...
    def _feature_coding(self, inputs, fname_temp):
        ...
        #normlize p3 and p2
        if torch.min(d) >= torch.min(d_p2): #2个数中取小的
            guiyihua_min = torch.min(d_p2)
        else:
            guiyihua_min = torch.min(d)
        if torch.max(d) >= torch.max(d_p2): #2个数中取大的
        ...
        d, h_new_p3_left, h_new_p3_right, w_new_p3_left, w_new_p3_right = Pfeature_zeropad_youxiajiao128(d, 16)
    ...
    def _evaluation(self, fname):
        ...
        features['p2'] = features_ds['p2']  #用31自己的P2
