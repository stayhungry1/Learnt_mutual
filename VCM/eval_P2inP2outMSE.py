...
class Eval:
    def __init__(self, settings, index) -> None:
        ...
        self.path_bppsave = 'output/cheng_onlycompressP2_bpp_lambda1e0.json'
        self.bpp_test5000 = {}
    ...
    def _feature_coding(self, inputs, fname_temp):
        ...
        temp_ori_size = d.shape #P2原始尺寸
        target_size = [d.size()[0], d.size()[1], padding_size(d.size()[2], 16), padding_size(d.size()[3], 16)] #P2补黑边后(16的倍数) [1, 256, 208, 304]
        d_big = torch.zeros(target_size).cuda()
        d_big[:, 0:temp_ori_size[1], 0:temp_ori_size[2], 0:temp_ori_size[3]] = d
        print(d_big.size(), '-------------------Cheng input (P2) size')
        d_output = torch.zeros(temp_ori_size) #用于从网络输出的tensor取出左上角
        net_belle_output = self.model.net_belle(d_big)
        print(net_belle_output["x_hat"].size(), '-------------------Cheng output (P2) size')
        d_output = net_belle_output["x_hat"][:, :, 0:temp_ori_size[2], 0:temp_ori_size[3]]
        print(d_output.size(), '-------------------output size')
        print('max/min_p2(GT)(Cheng input): %8.4f/%8.4f, max/min_P2(Cheng output): %8.4f/%8.4f' %(torch.max(d), torch.min(d), torch.max(d_output), torch.min(d_output)))
        features_cheng = features.copy()
        features_cheng["p2"] = d_output * guiyihua_scale + guiyihua_min
        ...
        out_criterion = self.criterion(net_belle_output, d_big, heigh_temp, width_temp)
        define_mse = nn.MSELoss()
        net_belle_output["x_hat"] = d_output #[1, 256, 208, 304]->[1, 256, 200, 304]
        out_criterion["mse_loss"] = define_mse(net_belle_output["x_hat"], d)
    ...
    def _evaluation(self, fname):
        ...
        fname_ds_rec = fname.replace('ori', 'ds')
