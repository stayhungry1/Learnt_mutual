...
class Worker(threading.Thread): #only P2_residual
...
class Worker_copy(threading.Thread):
    def __init__(self, file_path, path, qp):
        super().__init__()
        self.file_path = file_path
        self.path = path
...
def run_vtm(path, qp, threads): #only P2_residual
    ...
    for file_path in tqdm(FileList):
        ...
        if os.path.isfile(f"{resid_recon_path}{file_name}.png"):
...
def run_vtm_copy(path, qp, threads):
    FileList = glob.glob(f"{path}/*.png")
    bitstream_path = path.replace("ori", "bit/")
    recon_path = path.replace("ori", "rec/")
    temp_path = path.replace("ori", "tmp/")
